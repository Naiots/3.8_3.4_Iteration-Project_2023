		...
		if (isset($_POST['update'])) {
			...
			if ($NPW == '') { //Password is not updated
				...
				if ($Password == '') { //The current password has not been provided
					$check10 = "ERROR! Unable To Update Password.<br>Please provide your current password.<br>" . $stmt->error;
					$errorPassword = true;
					...
					if ($Password == $row['Password']) { //The current password provided is correct
						...
						$check10 = "ERROR! Unable To Update Password.<br>Invalid current password.<br>" . $stmt->error;
						$errorPassword = true;
					}
				}
			}
			...
			if ($updatequery != '') { // If one of the query were run, prepare the query
				...
				if ($stmt->execute()) { // If the preparation is executed, update the page and let the user know
					$check10 = "Succesfully Updated Your Details";
					// Fetch the existing record data to pre-fill the form
					$selectQuery = "SELECT * FROM users WHERE User_ID = ?";
					$stmt = $conn->prepare($selectQuery);
					$stmt->bind_param("i", $result2StringUserId); // "i" represents an integer
				...
				<div id = "content">
					...
					<div class="updateDetails">
						...
						<h3><form method = "post" id = "updateAccount">
							...
							<label for = "login" style = "padding-right: 21.5vw;">Current Password</label><br>
							...
							<label for = "login" style = "padding-right: 23.5vw;">Phone Number (Optional)</label><br>
							...
							<label for = "login" style = "padding-right: 23.5vw;">City (Optional)</label><br>
							...
							<label for = "country" style = "padding-right: 23.5vw;">Country (Optional)</label><br>
